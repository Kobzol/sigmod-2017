...
public:
    ...
    Job(size_t timestamp, std::string data) : timestamp(timestamp),
                                                        data(data)
    {
...
public:
    ...
    inline size_t get_available_job_count() const
    {
        size_t freeCount = this->index - this->workerIndex;
        return std::min(JOB_BATCH_SIZE, freeCount);
    }
